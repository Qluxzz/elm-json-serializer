...
if (!commandExists("elm-format")) {
  console.error(
    "Elm Format is not on your path. Install it before using this software please!"
  )
  process.exit(1)
}
...
program.on("--help", () => {
  console.log("\n  Examples:\n")
  console.log(
    "    $ elm-serializer --file example/Model.elm --type Model --output example\n"
  )
})
...
const elmOptions = (() => {
  try {
    ...
    console.error(
      `It looks like there's no elm.json where you are... Are you sure you're running this program at the root of an elm project?`
    )
    console.error(`Just in case, you're actually here: ${cwd}.`)
    console.error(
      `If you're sure to be inside an elm project, it looks like you need to run elm init before anything else!`
    )
    process.exit(1)
  }
})()
...
app.ports.writeFile.subscribe((decoderAndEncoder) => {
  if (decoderAndEncoder === null) {
    console.error(
      `${name} has not been found in the file you indicated. Please, check your settings.`
    )
    process.exit(1)
  }
